...
import (
	...
	pb "jugador2/proto/grpc-server/proto"
...
// Función para fusionar relojes vectoriales. (Se llama despues de recibir un mensaje de respuesta o notificación de otra entidad)
func (c *playerClient) mergeClocks(remoteClock []int32) {
	c.mu.Lock() //bloquea el reloj
	defer c.mu.Unlock()
	...
	for i := 0; i < len(c.vectorClock); i++ {
		if remoteClock[i] > c.vectorClock[i] { // compara cada reloj local con el reloj que acaba de recibir
...
func (c *playerClient) incrementClock() {
	c.mu.Lock() //1° bloquea el acceso al reloj
	defer c.mu.Unlock() // Incrementa únicamente su propio contador en el reloj vectorial. Si soy el Jugador 2 (playerIndex = 1) y mi reloj es [1, 5, 3, 0, 0, 0], después de esta función será [1, 6, 3, 0, 0, 0]
...
func main() {
	...
	// id del jugador
	playerId := "2"
	playerIndex := 1 // primer jugador en el reloj vectorial
